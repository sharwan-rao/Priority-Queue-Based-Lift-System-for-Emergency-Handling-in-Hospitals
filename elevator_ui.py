"""
Elevator UI: Modern Tkinter interface with elevator shaft visualization,
pending requests panel, speed controls, and statistics dashboard.
Features a dark theme with vibrant accents and smooth visual feedback.
"""
...
class ElevatorUI:
    ...
    def _create_control_panels(self, parent):
        ...
        for i, floor in enumerate(range(self.num_floors, 0, -1)):
            ...
            btn = tk.Button(internal_grid, text=str(floor), 
                           ...
                           bg=Colors.CARD_BG_LIGHT, fg=Colors.TEXT,
        ...
        # Emergency controls
        emergency_frame = tk.Frame(controls_container, bg=Colors.EMERGENCY_BG,
                                  highlightbackground=Colors.DANGER, highlightthickness=2)
        emergency_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        
        tk.Label(emergency_frame, text="🚨 Emergency Request", 
                font=('Segoe UI', 11, 'bold'),
                bg=Colors.EMERGENCY_BG, fg=Colors.DANGER).pack(anchor=tk.W, padx=15, pady=(10, 10))
        
        # From floor
        from_frame = tk.Frame(emergency_frame, bg=Colors.EMERGENCY_BG)
        from_frame.pack(fill=tk.X, padx=15, pady=5)
        tk.Label(from_frame, text="From Floor:", font=('Segoe UI', 10),
                bg=Colors.EMERGENCY_BG, fg=Colors.TEXT).pack(side=tk.LEFT)
        self.emergency_from = ttk.Combobox(from_frame, values=list(range(1, self.num_floors + 1)),
                                          width=5, state='readonly')
        self.emergency_from.set("1")
        self.emergency_from.pack(side=tk.RIGHT)
        
        # To floor
        to_frame = tk.Frame(emergency_frame, bg=Colors.EMERGENCY_BG)
        to_frame.pack(fill=tk.X, padx=15, pady=5)
        tk.Label(to_frame, text="To Floor:", font=('Segoe UI', 10),
                bg=Colors.EMERGENCY_BG, fg=Colors.TEXT).pack(side=tk.LEFT)
        self.emergency_to = ttk.Combobox(to_frame, values=list(range(1, self.num_floors + 1)),
                                        width=5, state='readonly')
        self.emergency_to.set("10")
        self.emergency_to.pack(side=tk.RIGHT)
        
        # Emergency button
        self.emergency_btn = tk.Button(emergency_frame, text="🚨 TRIGGER EMERGENCY",
                                       font=('Segoe UI', 11, 'bold'),
                                       bg=Colors.DANGER, fg=Colors.TEXT,
                                       relief='flat', cursor='hand2',
                                       activebackground=Colors.EMERGENCY_GLOW,
                                       command=self.handle_emergency)
        self.emergency_btn.pack(fill=tk.X, padx=15, pady=15)
    ...
    def handle_emergency(self):
        ...
        try:
            from_floor = int(self.emergency_from.get())
            to_floor = int(self.emergency_to.get())
        except ValueError:
            messagebox.showerror("Error", "Please select valid floors")
            return
        
        if from_floor == to_floor:
            messagebox.showwarning("Invalid", "From and To floors cannot be the same!")
            return
